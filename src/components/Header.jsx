      ...
      <header className="text-gray-600 body-font">
        <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center justify-between ps-16">
          <a className="flex title-font font-medium items-center text-gray-900 mb-4 md:mb-0">
            <img src={logo} alt="Imagen del Producto" className="w-12 h-12 text-white rounded-full" />
          ...
          <form className="block w-1/3">
            ...
            <div className="relative">
              ...
              <input type="search" id="default-search" className="block w-full p-4 ps-10 text-sm text-gray-900 border border-Azul-oscuro rounded-lg bg-gray-50 focus:border-Azul-oscuro" placeholder="Busca globos, decoraciones..." required />
          ...
          <div className='divide-x align-middle grid-flow-col'>
            <Link to="/carrito" className='align-middle'>
              <FontAwesomeIcon className='px-5 py-3 w-6 h-6' icon={faCartShopping} style={{ color: "#0081A7" }} />
            </Link>
          
           {sesionActiva ? (
              <span className="text-gray-900 mr-3">Bienvenido!</span>
            ) : (
              <Link to="/login" className='px-5 '>
                <button type="button" className="text-white bg-Azul-oscuro border-2 border-Azul-oscuro hover:text-Azul-oscuro hover:bg-Beige hover:w-30 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2">Login</button>
              </Link>
            )}
          </div>
        </div>
